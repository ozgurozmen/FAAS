...
void keygenPQ(
            ...
            int64_t     *g_inv,     /* output - secret key */
            int64_t     *h,         /* output - public key */
			int64_t     *h_inv,         /* output - public key */
            int64_t     *buf,       /* input  - buffer     */
    const   PQ_PARAM_SET*param)     /* input  - parameters */
{
    int64_t i;
    ...
    int64_t *gntt = buf  + param->padded_N;
    int64_t *hntt = gntt + param->padded_N;
	int64_t *hntt_inv = hntt + param->padded_N;
    int64_t *tmp  = hntt_inv + param->padded_N;  ;
    ...
    for (i=0;i<param->N;i++)
    {
        ...
        /* compute h = gf^-1 mod q */
        hntt[i] = gntt[i]*fntt[i] % param->q;
		hntt_inv[i] = InvMod(hntt[i], param->q);
    }

    Inv_NTT(param, h, hntt);
	//TAKE H INVERSE HERE! We may need to use InvMod?
//	int res;
//	do{
//	res = pol_inv_mod2(h_inv, h, param->N);
//	}while (res == -1);
	
	Inv_NTT(param, h_inv, hntt_inv);
